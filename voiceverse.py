...
def save_saved_conversations(conversations):
    try:
        with open(SAVED_HISTORY_FILE, "w") as f:
            json.dump(conversations[-5:], f, indent=2)   # keep only last 5
    except: pass
...
def detect_language(text):
    ...
    return "en"   # FIX: default English only — removed Portuguese heuristic

def get_ai_response(text, chat_history=None, lang_override=None):
    try:
        url     = "https://api.mistral.ai/v1/chat/completions"
        headers = {"Authorization": f"Bearer {MISTRAL_KEY}", "Content-Type": "application/json"}

        # FIX: language detection — only use script detection for non-Latin
        # If user typed/spoke English (Latin chars) → always respond in English
        if lang_override and lang_override != "auto":
            target_lang = lang_override
        else:
            target_lang = detect_language(text)   # script-based only
        ...
        system_content = (
            ...
            f"Keep response under 3 sentences. No bullet points."
        )
    ...
    return "I am unable to respond right now. Please try again."

def text_to_speech(text, lang_override=None):
    lang = lang_override if (lang_override and lang_override != "auto") else detect_language(text)
    supported = ["en","gu","hi","ar","bn","es","fr","de","ja","pt"]
    if lang not in supported: lang = "en"
...
def get_preview(conversation):
    """Return first few words of first user message — like Claude AI history."""
...
for k, v in {
    ...
    "all_conversations":     load_saved_conversations(),
}.items():
    if k not in st.session_state:
        st.session_state[k] = v

# Rebuild chat_messages from conversation if empty
if not st.session_state.chat_messages and st.session_state.conversation:
...
.block-container { padding: 0 2.5rem 4rem !important; max-width: 840px !important; margin: 0 auto !important; }
...
[data-testid="stSidebar"] div { color: #8892A4 !important; }
...
.vv-logo-ring {
    ...
    background: linear-gradient(145deg, #0f1535, #1a1060);
    box-shadow: 0 0 0 1px rgba(167,139,250,0.28), 0 0 45px rgba(124,58,237,0.55), inset 0 1px 0 rgba(255,255,255,0.06);
...
@keyframes logo-pulse {
    0%,100% { box-shadow: 0 0 0 1px rgba(167,139,250,0.28), 0 0 45px rgba(124,58,237,0.55), inset 0 1px 0 rgba(255,255,255,0.06); }
    50%      { box-shadow: 0 0 0 1px rgba(167,139,250,0.5),  0 0 65px rgba(124,58,237,0.75), inset 0 1px 0 rgba(255,255,255,0.06); }
}

/* Waveform bars inside logo */
.vv-waveform { display: flex; align-items: center; gap: 3px; height: 32px; }
.vv-bar {
    width: 4px; border-radius: 3px;
    background: linear-gradient(to top, #4F46E5, #A855F7);
    animation: bar-dance 1.4s ease-in-out infinite;
}
.vv-bar:nth-child(1) { height: 12px; animation-delay: 0s; }
.vv-bar:nth-child(2) { height: 22px; animation-delay: 0.15s; }
.vv-bar:nth-child(3) { height: 30px; animation-delay: 0.3s; }
.vv-bar:nth-child(4) { height: 22px; animation-delay: 0.45s; }
.vv-bar:nth-child(5) { height: 12px; animation-delay: 0.6s; }
@keyframes bar-dance {
...
.stat-chip { background: rgba(10,14,30,0.75); border: 1px solid rgba(99,102,241,0.14); border-radius: 14px; padding: 10px 22px; text-align: center; }
...
/* ── CHAT WINDOW ── */
.chat-window {
    position: relative; background: rgba(8,12,28,0.9);
    border: 1px solid rgba(99,102,241,0.14); border-radius: 28px; margin: 2rem 0 0; overflow: hidden;
    box-shadow: 0 0 0 1px rgba(255,255,255,0.03) inset, 0 24px 64px rgba(0,0,0,0.65);
...
.chat-messages {
    ...
    scroll-behavior: smooth; scrollbar-width: thin; scrollbar-color: rgba(99,102,241,0.2) transparent;
}
...
.empty-title { color:#6B7280;font-size:0.95rem;font-weight:600;text-align:center; }
.empty-hint  { color:#4B5563;font-size:0.78rem;text-align:center;line-height:1.5; }
...
.user-bubble { background:linear-gradient(145deg,#5B21B6,#7C3AED,#9333EA);color:#fff !important;padding:1rem 1.4rem;border-radius:20px 20px 4px 20px;max-width:76%;font-size:0.91rem;font-weight:500;line-height:1.65;word-break:break-word;box-shadow:0 8px 32px rgba(124,58,237,0.42); }
...
.ai-bubble { background:rgba(12,18,40,0.95);border:1px solid rgba(99,102,241,0.18);color:#D1D9E6 !important;padding:1rem 1.4rem;border-radius:4px 20px 20px 20px;font-size:0.91rem;line-height:1.72;word-break:break-word;box-shadow:0 8px 32px rgba(0,0,0,0.45); }
...
div[data-testid="stAudioInput"] {
    background: rgba(8,12,28,0.92) !important; border: 1px solid rgba(99,102,241,0.22) !important;
    border-radius: 22px !important; box-shadow: 0 8px 40px rgba(0,0,0,0.5) !important;
    transition: all 0.35s cubic-bezier(0.16,1,0.3,1) !important; overflow: hidden !important;
}
...
div[data-testid="stAudioInput"] > div, div[data-testid="stAudioInput"] > div > div, div[data-testid="stAudioInput"] > div > div > div { background: transparent !important; box-shadow: none !important; border-radius: 0 !important; }

/* ── ACTION BUTTONS ── */
div[data-testid="stHorizontalBlock"] div[data-testid="stButton"] button {
    background: rgba(239,68,68,0.06) !important; border: 1px solid rgba(239,68,68,0.18) !important;
    color: #F87171 !important; border-radius: 16px !important; font-size: 0.81rem !important;
...
if st.session_state.conversation:
    ...
    with col2:
        # FIX: New Chat — completely fresh, NO last message carried over
        if st.button("✨ New Chat", use_container_width=True, type="primary"):
            ...
            # Completely wipe all conversation state
            st.session_state.conversation        = []
            st.session_state.chat_messages       = []
            st.session_state.current_user_text   = ""
            st.session_state.current_ai_text     = ""
...
with st.sidebar:
    ...
    lang_labels  = {"auto":"🤖 Auto-detect","en":"🇬🇧 English","gu":"🇮🇳 Gujarati","hi":"🇮🇳 Hindi","ar":"🇸🇦 Arabic","bn":"🇧🇩 Bengali","pt":"🇵🇹 Portuguese","es":"🇪🇸 Spanish","fr":"🇫🇷 French","de":"🇩🇪 German","ja":"🇯🇵 Japanese"}
    selected_lang = st.selectbox("lang", options=lang_options, format_func=lambda x: lang_labels.get(x,x),
                                  index=lang_options.index(st.session_state.selected_language), label_visibility="collapsed")
    if selected_lang != st.session_state.selected_language:
        if st.session_state.conversation:
            saved = st.session_state.all_conversations
            saved.append({"timestamp":datetime.now().strftime("%I:%M %p, %d %b"),
                          "conversation":st.session_state.conversation.copy(),
                          "messages":st.session_state.chat_messages.copy(),
                          "preview":get_preview(st.session_state.conversation)})
            st.session_state.all_conversations = saved[-5:]
    ...
    # History — FIX: preview text instead of timestamp, max 5, proper spacing
    ...
    if st.session_state.all_conversations:
        ...
        for idx, conv in enumerate(recent):
            ...
            # FIX: show preview text (first few words) like Claude AI
